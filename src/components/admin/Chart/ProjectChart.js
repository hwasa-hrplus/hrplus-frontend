...
class ProjectChart extends Component {
    ...
    getMyData = async () => {
        ...
        for (let idx = 0; idx < this.state.employeeData.length; idx++) {
            const projectName = this.state.employeeData[idx].project.name;
        ...
        for (let idx = 0; idx < uniqueDataNameArr.length; idx++){
            let cnt = this.state.employeeData.filter(data =>
                uniqueDataNameArr[idx] === data.project.name).length;
                        ...
                        <TableBody>
                            {   
                                this.state.employeeData.filter((data) =>{
                                    if (this.state.searchingKeyword === "사원 이름을 입력하세요." && data.project.name === this.state.dataName && this.state.dataName){
                                        console.log('클릭로직');
                                        console.log('data.project.name: ', data.project.name);
                                        return data;
                                    }
                                    else if (data.project.name.includes(this.state.dataName) && data.korName.toLowerCase().includes(this.state.searchingKeyword.toLowerCase())){
                                        console.log('검색로직');
                                        return data;
                                    }
                                    else {
