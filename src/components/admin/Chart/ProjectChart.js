...
class ProjectChart extends Component {
    ...
    findTableByChartClick = (data) => {
        ...
        console.log(`${data.name} 차트 조각 클릭!`);
    }
    ...
    requestData = async () => {
        ...
        console.log('employeeDataSorted: ', employeeDataSorted);
        ...
        for (let idx = 0; idx < this.state.employeeData.length; idx++) {
            const projectName = this.state.employeeData[idx].code;
        ...
        for (let idx = 0; idx < uniqueDataNameArr.length; idx++){
            let cnt = this.state.employeeData.filter(data =>
                uniqueDataNameArr[idx] === data.code).length;
            console.log(`${uniqueDataNameArr[idx]} :${cnt}명`);
            let uniqueObj = {};
    ...
    handleClick = () => {
        let filteredData = this.state.employeeData.filter((data) =>{                   
            ...
            else if (data.code.includes(this.state.dataName) && data.korName.includes(this.state.searchingKeyword)){
                console.log('검색로직');
                return data;
            }
            else {
        ...
        return (
            <div>
                ...
                <div className="ContentWrapper">
                    ...
                    <div>
                        <h3 style={{padding: 10}}>선택 프로젝트: {this.state.dataName ? this.state.dataName : "None"}</h3>
                    </div>
                </div>
